...
router.get('/', requireAuth, async (req, res) => {
  const r = await db.query(
    `SELECT u.id,u.username,u.display_name,u.avatar,u.status,u.badge_blue,u.badge_gold,u.badge_rail
     FROM friends f JOIN users u ON u.id=f.friend_id WHERE f.user_id=$1 AND u.is_banned=FALSE ORDER BY u.username`,
    [req.session.userId]
  );
  res.json(r.rows);
});
...
router.get('/requests', requireAuth, async (req, res) => {
  const incoming = await db.query(
    `SELECT fr.id,fr.sender_id,fr.created_at,u.username,u.display_name,u.avatar
     FROM friend_requests fr JOIN users u ON u.id=fr.sender_id WHERE fr.receiver_id=$1 AND fr.status='pending' AND u.is_banned=FALSE`,
    [req.session.userId]
  );
  const outgoing = await db.query(
    `SELECT fr.id,fr.receiver_id,fr.created_at,u.username,u.display_name,u.avatar
     FROM friend_requests fr JOIN users u ON u.id=fr.receiver_id WHERE fr.sender_id=$1 AND fr.status='pending'`,
    [req.session.userId]
  );
...
router.post('/request', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // If they already sent one, auto-accept
    const theirReq = await db.query(`SELECT id FROM friend_requests WHERE sender_id=$1 AND receiver_id=$2 AND status='pending'`, [userId, req.session.userId]);
    if (theirReq.rows.length) {
      await acceptFriendRequest(theirReq.rows[0].id, req.session.userId, parseInt(userId));
      return res.json({ message: 'Friend request accepted!' });
    }
    const dup = await db.query(`SELECT id FROM friend_requests WHERE sender_id=$1 AND receiver_id=$2 AND status='pending'`, [req.session.userId, userId]);
    if (dup.rows.length) return res.status(409).json({ error: 'Request already sent' });
    await db.query(`INSERT INTO friend_requests (sender_id,receiver_id) VALUES ($1,$2)`, [req.session.userId, userId]);
...
async function acceptFriendRequest(requestId, myId, theirId) {
...
router.post('/requests/:id/accept', requireAuth, async (req, res) => {
  ...
  await acceptFriendRequest(parseInt(req.params.id), req.session.userId, sender_id);
