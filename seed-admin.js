...
async function seed() {
  ...
  try {
    ...
    if (existing.rows.length) {
      await client.query(`UPDATE users SET password_hash=$1,email_verified=TRUE,is_admin=TRUE,badge_blue=TRUE,badge_gold=TRUE,badge_admin=TRUE WHERE LOWER(username)='admin'`, [hash]);
      console.log('✅ Admin account updated — Username: Admin | Password: whatthesigma');
    } else {
      await client.query(
